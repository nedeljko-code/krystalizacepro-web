...
const ServiceSingle = async (props: {
  ...
  const { locale, single } = await props.params;

  const { card_ui,product_card } = getListPage("services/-index.md", locale).frontmatter;

  const allServices: Service[] = getSinglePage("services", locale);
  const service = allServices.find((page) => page.slug === single);
              ...
              <div className="lg:sticky lg:top-24">
                ...
                <div className="overflow-hidden rounded-2xl border border-border bg-white shadow-sm">
                  <ProductCard
                    ...
                    documentationHref={`/${locale}/dokumentace#${service.slug}`}
                    contactLink={`/${locale}/${config.navigation_button.link.replace(/^\/+/, "")}`}
                    labels={product_card}
                  />
                </div>
              </div>
                ...
                return (
                  ...
                  >
                    <ServiceCard
                      service={product}
                      locale={locale}
                      comingSoonLabel={card_ui.coming_soon}
                      viewProductLabel={card_ui.view_product}
                    />
                  </div>
                );
              })}
